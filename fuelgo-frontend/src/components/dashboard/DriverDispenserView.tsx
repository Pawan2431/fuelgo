                  ...
                  {isCompleted && (
                    <div className="w-full flex flex-col space-y-3">
                      {activeOrder.paymentMethod === 'cod' && (
                        <div className={`p-4 rounded-xl border ${activeOrder.paymentStatus === 'paid' ? 'bg-emerald-50 border-emerald-200' : 'bg-amber-50 border-amber-200'}`}>
                          <div className="flex justify-between items-center mb-3">
                            <div>
                              <span className="text-[11px] font-bold uppercase tracking-wider block mb-0.5 text-gray-500">COD ORDER</span>
                              <span className="text-sm font-bold text-gray-900">
                                {activeOrder.paymentStatus === 'paid' ? 'Payment Collected' : 'Amount to Collect'}
                              </span>
                            </div>
                            <div className={`text-xl font-mono font-bold ${activeOrder.paymentStatus === 'paid' ? 'text-emerald-700' : 'text-amber-700'}`}>
                               ₹{activeOrder.totalPayable?.toLocaleString('en-IN') || (activeOrder.quantity * 92.5).toLocaleString('en-IN')}
                            </div>
                          </div>
                          {activeOrder.paymentStatus !== 'paid' && (
                            <button
                              type="button"
                              onClick={() => markOrderPaid(activeOrder.id)}
                              className="w-full py-3 bg-amber-600 hover:bg-amber-700 text-white font-bold rounded-xl text-sm flex items-center justify-center space-x-2 shadow-sm transition-all"
                            >
                      ...
                      <button
                        ...
                        className="w-full py-3 bg-gray-900 hover:bg-gray-800 text-white font-bold rounded-xl text-sm flex items-center justify-center space-x-1.5 shadow-sm"
                      >
