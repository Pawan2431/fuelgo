...
const dbFile = process.env.DB_FILE || path.join(__dirname, '../database/fuelgo.db');
...
// ── UPI Manual Payment Tables ──
db.exec(`
  CREATE TABLE IF NOT EXISTS merchant_config (
    id INTEGER PRIMARY KEY DEFAULT 1,
    merchant_name TEXT NOT NULL DEFAULT 'FuelGo India',
    merchant_upi_id TEXT NOT NULL DEFAULT 'fuelgo@upi',
    merchant_qr_data TEXT,
    updated_at DATETIME DEFAULT CURRENT_TIMESTAMP,
    updated_by TEXT
  );
  ...
  CREATE TABLE IF NOT EXISTS upi_payments (
    ...
    payment_status TEXT NOT NULL DEFAULT 'PAYMENT_VERIFICATION_PENDING',
...
// Seed default merchant config if not exists
const merchantExists = db.prepare('SELECT COUNT(*) as count FROM merchant_config').get();
if (merchantExists.count === 0) {
  db.prepare('INSERT INTO merchant_config (id, merchant_name, merchant_upi_id) VALUES (1, ?, ?)').run('FuelGo India', 'fuelgo@upi');
}
